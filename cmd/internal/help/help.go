...
Args:
	for i, arg := range args {
		...
		helpSuccess := "gopspx help"
		if i > 0 {
			helpSuccess += " " + strings.Join(args[:i], " ")
		}
		fmt.Fprintf(os.Stderr, "gopspx help %s: unknown help topic. Run '%s'.\n", strings.Join(args, " "), helpSuccess)
		os.Exit(2)
	}
...
Use "gopspx help{{with .LongName}} {{.}}{{end}} <command>" for more information about a command.
