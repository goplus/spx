...
func main() {
	...
	if err != nil {
		...
		status = commandErrorStatus(status, err)
	}
	xgolauncher.Exit(status)
}

func commandErrorStatus(status xgolauncher.ProcessStatus, err error) xgolauncher.ProcessStatus {
	if !status.Success() {
		return status
	}
	if xgodriver.IsRequestError(err) {
		return xgolauncher.ProcessStatus{Code: 2}
	}
	return xgolauncher.ProcessStatus{Code: 1}
}
