...
- `spx-runtime-assets.zip` is a separate SPX runtime pack generated by the dedicated pack-only desktop command (`spx exportpack`), the project template, the SPX Go runtime, and the byte-producing `runtime export-pack` path. The pack-only command writes `.builds/pc/gdexport.pck` without requiring a platform export template. Independent run, launcher, Web/mobile export, and release-orchestration changes do not invalidate it.
