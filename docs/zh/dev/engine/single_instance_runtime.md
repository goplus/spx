...
5. `Game.OnEngineStart` 创建受管理的启动协程，加载资源并启动脚本。只有成功时才调用 `OnGameStarted` / `OnInited`。
...
如果排空超时，binding 保持关闭，继续异步等待原有工作结束。`OnEngineDestroyed` 只设置 `backendDestroyed`。只有两者都完成，才 unlink 并释放单实例槽位。超时不会提前允许新 Game 启动。
