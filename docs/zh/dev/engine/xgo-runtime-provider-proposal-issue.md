...
为 XGo class project 引入 framework-owned、project-scoped、版本化的 runtime provider。framework 在自己的 `gox.mod` 中声明 provider；XGo 根据应用的有效 Go module/workspace graph 找到并构建它，然后把 `run` 或 transactional `build` 委托给 provider；`install` 复用 `build`。
...
- runtime provider 与 class metadata 来自同一份有效 Go graph；
- `xgo run` 不生成 `xgo_autogen.go`，也不向项目写入 `.temp`、`.godot` 或构建中间物；
- `xgo build` 生成可离线运行的 host 平台单文件程序；
- 未声明 runtime 的项目完全沿用现有 GenGo 路径；
- 一旦匹配 runtime，后续错误必须终止，不得静默回退。

## 设计原则

1. **Framework owns policy**：framework 决定如何运行和打包自己的项目，XGo 只提供生命周期。
2. **One graph, one identity**：metadata、provider package、bridge 源码都由同一有效 module/workspace graph 决定。
3. **Fail closed after match**：只有明确未匹配 runtime 时才允许进入旧路径。
...
| 组件 | 职责 |
| --- | --- |
| `goplus/mod` | `runtime` metadata、resolved module provenance、共享的 typed request 与 argv codec |
| `goplus/xgo` | graph/target 解析、provider 发现与构建、协议调用、信号转发、build/install 输出事务 |
| `goplus/spx` | SPX 协议适配、Engine/bridge/project bundle、解释执行、缓存和自包含 launcher |

共享层只定义 provider-neutral 数据，不提供 provider lifecycle SDK，也不包含 SPX domain rule。
...
`runtime` 作用于它前面最近的 `project`，每个 project 最多声明一个。语法必须是：
...
- `protocol` 当前只执行 `v1`；
- provider 必须是合法 Go import path，不允许相对路径、绝对路径或 `path@version`；
...
runtime discovery 位于 target 解析之后、所有 Dir/PkgPath/Files 分支及 GenGo 之前。
...
`install` 复用 `build` 语义并安装到有效 `GOBIN`；`GOBIN` 为空时使用 `GOPATH` 第一项下的 `bin`。runtime v1 一次只接受一个 install target。
...
3. provider identity：package、selected/replacement provenance、declaration path 与摘要；
...
codec 对未知、重复、缺失、组合不完整或 action 不适用的字段统一报错。路径在共享层做结构校验，在 provider 层绑定真实文件身份。协议 argv 与环境总大小超过平台安全预算时，在启动 provider 前失败。

XGo 当前只向 runtime provider 传递以下策略：
...
provider 保留 `SessionDir` 的 `--path` 控制权，用户不能覆盖它。每次 run 创建新 session，准备 bridge/Engine 配置后启动 Engine；项目目录在成功和失败路径中都不应发生变化。
...
以下输入都视为不可信：module/workspace metadata、provider argv、环境变量、release manifest、ZIP/payload、项目资源索引、cache 内容和已有输出路径。

边界统一遵循：

- canonical path 与真实文件身份同时校验，不只比较字符串；
- runtime request 与各类 manifest 使用严格解析，拒绝未知或重复字段；
...
- provider、Engine 和 launcher 的子进程由 supervisor 管理，取消后不得遗留子进程；
- runtime 路径绝不回退到 native/GenGo。
...
5. 最后发布包含 `runtime v1` 声明的新 SPX module 版本。
