...
func Gopt_Game_Run(game Gamer, resource any, gameConf ...*Config) {
	...
	if g.isAutoSetCollisionLayer {
		...
		idx := 0
		if len(g.typs) > 62 {
			panic(fmt.Sprintf("In auto set collision layer mode, sprite count should <= 62, current count = %d", len(g.typs)))
		}
		for name, _ := range g.typs {
			g.sprCollisionInfos[name] = &spriteCollisionInfo{Id: idx, Layer: 1 << idx}
			idx++
		}
	}
