...
func (p *Game) SetupPathFinder__1(x_grid_size, y_grid_size, x_cell_size, y_cell_size float64, with_jump, with_debug bool) {
	p.rt().navigationMgr.SetupPathFinderWithSize(mathf.NewVec2(x_grid_size, y_grid_size), mathf.NewVec2(x_cell_size, y_cell_size), with_jump, with_debug)
}
...
func (p *Game) setupPathFinder(with_jump, with_debug bool) {
	...
	p.rt().navigationMgr.SetupPathFinderWithSize(gridSize, cellSize, with_jump, with_debug)
}

func (p *Game) setObstacle(sprite Sprite, enabled bool) {
	impl := spriteOf(sprite)
	if impl != nil {
		p.rt().navigationMgr.SetObstacle(impl.getSpriteId(), enabled)
	}
}
...
func (p *Game) FindPath__2(x_from, y_from, x_to, y_to float64, with_debug, with_jump bool) []float64 {
	...
	arr := p.rt().navigationMgr.FindPath(mathf.NewVec2(x_from, y_from), mathf.NewVec2(x_to, y_to), with_jump)
	result := arr.([]float32)
