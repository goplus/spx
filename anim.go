...
// Animations
// speedRatio 1.0
func PlayAnimation(g *Game, target IAnimationTarget, from int, to int, loop bool, speedRatio float64) {

	// Local animations
	if target.GetAnimations() != nil {

		StopAnimation(g, target)

		animatable := NewAnimatable(target, from, to, loop, speedRatio)
		g.activeAnimatables = append(g.activeAnimatables, animatable)
	}
	...
	if target.GetAnimatables() != nil {
		...
		for index := 0; index < len(animatables); index++ {
			PlayAnimation(g, animatables[index].GetTarget(), from, to, loop, speedRatio)
		}
	}
}
func StopAnimation(g *Game, target IAnimationTarget) {
	for index := 0; index < len(g.activeAnimatables); index++ {
		if reflect.DeepEqual(g.activeAnimatables[index].GetTarget(), target) {
			g.activeAnimatables = append(g.activeAnimatables[:index], g.activeAnimatables[index+1:]...)
			return
		}
	}
}
