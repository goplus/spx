...
// ModuleRef and ModuleOrigin are XGo's resolved graph identities.
...
type goCommandGraph struct {
	goWork         string
	flags          []string
	privateModFile string
}

// Config is SPX's domain view of a driver request.
type Config struct {
	Action          Action
	ProjectDir      string
	ProjectFile     string
...
func (p Config) goWorkForCommand() string {
	if p.commandGraph != nil {
		return p.commandGraph.goWork
	}
	return p.GoWork
}

func (p Config) graphFlagsForCommand() []string {
	if p.commandGraph != nil {
		return p.commandGraph.flags
	}
	return p.GraphFlags
}

func (p Config) privateModFileForCommand() string {
	if p.commandGraph != nil {
		return p.commandGraph.privateModFile
	}
	return ""
}

// Parse decodes a request and verifies its live SPX inputs.
func Parse(args []string) (Config, error) {
	...
	cfg := Config{
		...
		TargetModFile: request.TargetModFile,
		Project: ProjectMetadata{
...
// validateLive binds the decoded request to its filesystem inputs.
func (p Config) validateLive() error {
	for _, item := range []struct {
		...
		{name: "target-modfile", value: p.TargetModFile.Path},
	} {
...
// validateCanonicalExistingPath checks XGo's canonical-path claim.
