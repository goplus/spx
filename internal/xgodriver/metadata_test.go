...
func TestVerifyDeclarationRejectsInvalidInputs(t *testing.T) {
	...
	tests := []struct {
		...
		{"missing", xgomod.FileIdentity{Path: filepath.Join(dir, "missing"), SHA256: hex.EncodeToString(validDigest[:])}, "lstat declaration"},
...
func TestVerifyTargetModFile(t *testing.T) {
	...
	digest := sha256.Sum256(data)
	identity := xgomod.FileIdentity{Path: path, SHA256: hex.EncodeToString(digest[:])}
	if err := VerifyTargetModFile(identity); err != nil {
		t.Fatalf("VerifyTargetModFile() error: %v", err)
	}
	if err := os.WriteFile(path, append(data, '\n'), 0o600); err != nil {
		t.Fatal(err)
	}
	if err := VerifyTargetModFile(identity); err == nil || !strings.Contains(err.Error(), "target modfile") {
