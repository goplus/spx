...
func TestBuildToMatchesBuildBytes(t *testing.T) {
	...
	want, wantPayloadDigest, wantManifestDigest, err := Build(config)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEstimatePayloadArchiveSizeMatchesBuild(t *testing.T) {
	payload, _, _, err := Build(testBuildConfig(t))
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildVerifyCanonicalPayload(t *testing.T) {
	config := testBuildConfig(t)
	first, firstPayloadDigest, firstManifestDigest, err := Build(config)
	if err != nil {
		t.Fatal(err)
	}
	second, secondPayloadDigest, secondManifestDigest, err := Build(config)
	if err != nil {
		t.Fatal(err)
	}
	...
	project := verified.ProjectZIP()
	project[0] ^= 0xff
	if bytes.Equal(project, verified.ProjectZIP()) {
		t.Fatal("ProjectZIP returned mutable internal storage")
	}
	engineZIP, err := verified.ComponentZIP("engine/")
	if err != nil {
		t.Fatal(err)
	}
	engine, err := ComponentBundle(engineZIP, runtimebundle.NamespaceEngine)
	if err != nil {
		t.Fatal(err)
	}
...
func TestVerifyRejectsDigestTargetAndManifestTampering(t *testing.T) {
	payload, payloadDigest, manifestDigest, err := Build(testBuildConfig(t))
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildRejectsInvalidLayoutInputs(t *testing.T) {
	...
	for _, test := range []struct {
		...
		{name: "manifest reserved", mutate: func(config *BuildConfig) {
			config.Files[0].Name = ManifestPath
		}},
		{name: "duplicate", mutate: func(config *BuildConfig) {
			config.Files[1].Name = config.Files[0].Name
		}},
		{name: "traversal", mutate: func(config *BuildConfig) {
			config.Files[0].Name = "../engine"
		}},
		{name: "bad digest", mutate: func(config *BuildConfig) {
			config.Engine.BundleDigest = "not-a-digest"
		}},
		{name: "bad pack", mutate: func(config *BuildConfig) {
			config.Project.PackDirectory = "../assets"
		}},
	} {
		t.Run(test.name, func(t *testing.T) {
			config := base
			config.Files = append([]File(nil), base.Files...)
			test.mutate(&config)
			if _, _, _, err := Build(config); err == nil {
...
func fileSources(config BuildConfig) (BuildConfig, []FileSource) {
...
func configFileData(t *testing.T, config BuildConfig, name string) []byte {
	t.Helper()
...
func testBuildConfig(t *testing.T) BuildConfig {
	...
	projectZIP, err := CanonicalComponentZIP([]File{
	...
	bridge, err := ComponentBundle(bridgeZIP, runtimebundle.NamespaceBridge)
	if err != nil {
		t.Fatal(err)
	}
	project, err := ComponentBundle(projectZIP, runtimebundle.NamespaceProject)
	if err != nil {
		t.Fatal(err)
	}
...
func rewritePayloadManifest(t *testing.T, payload []byte, mutate func(*Manifest)) ([]byte, string, string) {
	...
	for _, file := range reader.File {
		...
		files = append(files, File{Name: file.Name, Mode: file.Mode(), Data: data})
	}
	mutate(&manifest)
	...
	files = append(files, File{Name: ManifestPath, Mode: 0o644, Data: manifestData})
