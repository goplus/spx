...
func AddToGoSource(src []byte) []byte {
	...
	if Has(src) {
		return append(prefix, src...)
	}

	buildPrefix, rest := splitBuildTagPrefix(src)

	out := make([]byte, 0, len(prefix)+len(buildPrefix)+len(headerBytes)+len(rest))
...
func AddToGoFile(path string) error {
	data, err := os.ReadFile(path)
	if err != nil {
		return err
	}
	updated := AddToGoSource(data)
