...
class GameApp {
	constructor(config) {
		...
		// Initialize worker messaging only when the engine runtime actually uses worker mode.
		this.workerMessageManager = this.workerMode ? new globalThis.WorkerMessageManager() : null;
	...
	callWorkerFunction(funcName, ...args) {
		if (this.workerMessageManager == null) {
			throw new Error(`worker bridge "${funcName}" is unavailable in non-worker mode`)
		}
