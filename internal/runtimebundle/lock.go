...
// SharedLeaseProvider names the same shared/exclusive contract for callers
// that previously kept it separate from publication locking.
type SharedLeaseProvider = LockProvider
...
// OSLockProvider is a descriptive alias for CrossProcessLockProvider.
type OSLockProvider = CrossProcessLockProvider

// Acquire is a compatibility spelling for an exclusive publication lock.
func (CrossProcessLockProvider) Acquire(ctx context.Context, key string) (LockLease, error) {
...
func (UnsupportedCrossProcessLockProvider) Acquire(ctx context.Context, key string) (LockLease, error) {
...
func (ProcessLockProvider) Acquire(ctx context.Context, key string) (LockLease, error) {
