...
func TestParseRejectsInvalidArgv(t *testing.T) {
	tests := []struct {
		...
		{"partial replacement", func(t *testing.T) []string { return append(validArgs(t, ActionRun), "--replace-path=local", "--") }, "complete group"},
		{"mixed replacement source", func(t *testing.T) []string {
			args := validArgs(t, ActionRun)
			root := filepath.Dir(optionValue(args, "project-dir"))
		...
		}, "forbids --selected-dir"},
		{"run output", func(t *testing.T) []string { return append(validArgs(t, ActionRun), "--output=/tmp/out", "--") }, "does not accept --output"},
		{"build missing output", func(t *testing.T) []string { return validArgs(t, ActionBuild) }, "requires --output and --final-output"},
