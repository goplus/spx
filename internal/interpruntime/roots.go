...
func portableConfigValueFromEnv(env []string, wanted string) (string, bool, error) {
	var value string
	found := false
	for _, entry := range env {
		...
		canonical, ok := rootEnvKey(key)
		if !ok || canonical != wanted {
			continue
		}
		if found {
			return "", false, fmt.Errorf("interpruntime: duplicate environment variable %s", wanted)
		}
