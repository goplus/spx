...
func TestMainExecutionScopeRestoresEnclosingExecution(t *testing.T) {
	...
	endOuter := thread.BeginMainExecution(outerStartedAt)
	endInner := thread.BeginMainExecution(innerStartedAt)
	if got := thread.MainExecutionStartedAt(); !got.Equal(innerStartedAt) {
		t.Fatalf("inner Main started at %v, want %v", got, innerStartedAt)
	}
	thread.DemoteMainExecution()
	if got := thread.MainExecutionStartedAt(); !got.IsZero() {
		t.Fatalf("demoted inner Main started at %v, want zero", got)
	}

	endInner()
	if got := thread.MainExecutionStartedAt(); !got.Equal(outerStartedAt) {
