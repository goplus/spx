module github.com/goplus/spx/v2

go 1.25.0

tool github.com/goplus/ixgo/cmd/qexp

require (
	github.com/goplus/ixgo v1.1.1-0.20260622131723-afe6b618ba9e
	github.com/goplus/mod v0.21.1
	github.com/goplus/reflectx v1.7.0
	github.com/goplus/spbase v0.1.0
	github.com/goplus/xgo v1.7.4-0.20260627135303-d8554334d6f2
	github.com/petermattis/goid v0.0.0-20250721140440-ea1c0173183e
	golang.org/x/mobile v0.0.0-20220518205345-8578da9835fd
	golang.org/x/mod v0.32.0
)

require (
	github.com/goplus/gogen v1.23.4-0.20260623084408-4e0857017abd // indirect
	github.com/qiniu/x v1.18.0 // indirect
	github.com/timandy/routine v1.1.6 // indirect
	github.com/visualfc/funcval v0.1.4 // indirect
	github.com/visualfc/gid v0.3.1 // indirect
	github.com/visualfc/xtype v0.3.0 // indirect
	golang.org/x/image v0.23.0 // indirect
	golang.org/x/tools v0.41.0 // indirect
)
