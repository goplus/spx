...
static volatile int64_t last_second_id;
struct ScalarSink {
    ...
    void set_camera_limit(int64_t side, int64_t limit) { last_id = side; last_second_id = limit; }
    void set_z_index(int64_t id, int64_t z) { last_id = id; last_second_id = z; }
    uint8_t check_collision(int64_t id, int64_t target, uint8_t source_trigger, uint8_t target_trigger) {
        last_id = id;
        last_second_id = target;
        return id != target && source_trigger != target_trigger;
    }
...
EMSCRIPTEN_KEEPALIVE uint32_t benchmark_second_id_low() { return uint32_t(last_second_id); }
EMSCRIPTEN_KEEPALIVE uint32_t benchmark_second_id_high() { return uint64_t(last_second_id) >> 32; }
