...
const cases = [
    ...
    ['two 64-bit integers', 1, api => api.gdspx_camera_set_camera_limit(1, 0x200000, 0xffffffff, 0xffffffff)],
    ['64-bit ID + integer', 1, api => api.gdspx_sprite_set_z_index(1, 0x200000, 0xffffffff, 0xffffffff)],
    ['two 64-bit IDs + bool result', 1, api => api.gdspx_sprite_check_collision(1, 0x200000, 0, 0x80000000, true, false)],
...
function verify(module, api) {
    ...
    for (const [low, high] of [[0, 0], [1, 0], [1, 0x200000], [0xffffffff, 0x7fffffff], [0, 0x80000000], [0xffffffff, 0xffffffff]]) {
        ...
        for (const name of ['gdspx_camera_set_camera_limit', 'gdspx_sprite_set_z_index']) {
            api[name](low, high, 0xffffffff, 0xffffffff);
            assert.equal(module._benchmark_id_low() >>> 0, low);
            assert.equal(module._benchmark_id_high() >>> 0, high);
            assert.equal(module._benchmark_second_id_low() >>> 0, 0xffffffff);
            assert.equal(module._benchmark_second_id_high() >>> 0, 0xffffffff);
        }
        assert.equal(api.gdspx_sprite_check_collision(low, high, low ^ 1, high, true, false), true);
        assert.equal(module._benchmark_id_low() >>> 0, low);
        assert.equal(module._benchmark_id_high() >>> 0, high);
        assert.equal(module._benchmark_second_id_low() >>> 0, (low ^ 1) >>> 0);
        assert.equal(module._benchmark_second_id_high() >>> 0, high);
    }
